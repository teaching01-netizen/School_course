...
cleanup() {
  ...
  if [[ -n "${legacy_sync_pid:-}" ]] && kill -0 "$legacy_sync_pid" 2>/dev/null; then
    echo "[dev] stopping legacy-sync worker (pid=$legacy_sync_pid)..."
    kill -INT "$legacy_sync_pid" 2>/dev/null || true
  fi
  ...
  if [[ -n "${legacy_sync_pid:-}" ]]; then
    wait "$legacy_sync_pid" 2>/dev/null || true
  fi
}
...
echo "[dev] starting legacy-sync worker..."
(cd backend && make legacy-sync) &
legacy_sync_pid="$!"
