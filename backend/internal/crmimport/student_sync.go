...
func (s *StudentSyncService) SyncFromSnapshot(ctx context.Context, snapshotID pgtype.UUID) (int, error) {
	...
	for rows.Next() {
		var sr studentRow
		if err := rows.Scan(&sr.WCode, &sr.FullName, &sr.Nickname); err != nil {
	...
	for i := 0; i < len(students); i += batchSize {
		...
		res, err := tx.Exec(ctx, `
			...
			SELECT ss.wcode, ss.full_name, '' FROM _sync_students ss
...
func studentCopies(students []studentRow) [][]any {
	...
	for i, s := range students {
		sources[i] = []any{s.WCode, s.FullName}
	}
	return sources
}
