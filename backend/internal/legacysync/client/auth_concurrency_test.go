...
func TestClient_ConcurrentReauthFailureIsBounded(t *testing.T) {
	// The session-expiry convoy stays bounded at every concurrency ceiling:
	// however many requests hit the expired session simultaneously, the
	// generation check allows exactly one re-login chain, then each request
	// retries once against the failed auth and surfaces ErrAuthentication.
	for _, maxConcurrent := range []int{10, 32, 128} {
