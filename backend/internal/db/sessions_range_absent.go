...
// sessionsRangeAbsentSQLText is the already-absent SELECT shared by the
// standalone probe and the tail batch (ScopesCountsAbsentBlockedBatch).
// Byte-identical predicate; params $1=wcode, $2=TZ, $3=FromUTC,
// $4=ToExclusiveUTC.
func sessionsRangeAbsentSQLText() string {
	return `
...
// sessionsRangeAbsentSQL queues the same text on a batch.
func sessionsRangeAbsentSQL() string { return sessionsRangeAbsentSQLText() }
...
func (q *Queries) SessionsRangeAlreadyAbsent(ctx context.Context, arg SessionsRangeAbsentParams) (map[string]bool, error) {
	rows, err := q.db.Query(ctx, sessionsRangeAbsentSQLText(), arg.Wcode, arg.InstituteTZ, arg.FromUTC, arg.ToExclusiveUTC)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	return scanAbsentRows(rows)
}

// scanAbsentRows drains an already-absent result set (standalone or the
// tail batch's second statement) into the session-ID set.
func scanAbsentRows(rows pgx.Rows) (map[string]bool, error) {
