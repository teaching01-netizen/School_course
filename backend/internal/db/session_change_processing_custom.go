...
func (q *Queries) ScheduleImpactProcessing(ctx context.Context, limit int32) ([]ScheduleImpactProcessingRow, error) {
	rows, err := q.db.Query(ctx, `
		SELECT sc.id, COALESCE(c.code, ''), COALESCE(c.name, ''), sc.created_at, run.status, run.last_error
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.CourseCode, &item.CourseName, &item.CreatedAt, &item.Status, &item.LastError); err != nil {
			return nil, err
		}
