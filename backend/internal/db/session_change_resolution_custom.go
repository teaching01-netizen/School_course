...
// ResolveScheduleIssue resolves a schedule issue with a given action.
//
// Deprecated: Use ResolveScheduleIssueWithSnapshot instead. This method does not
// populate snapshot columns on new sit-in assignments created during reassignment.
...
// ResolveScheduleIssueWithSnapshot resolves a schedule issue, building session
// snapshots for any new sit-in assignments created during reassignment.
func (q *Queries) ResolveScheduleIssueWithSnapshot(ctx context.Context, issueID, candidateSessionID, actorID pgtype.UUID, expectedIssueVersion, expectedSessionVersion int32, action, reason string, timezone string, snapshotFunc SnapshotBuilderFunc) (string, error) {
	action = strings.ToLower(strings.TrimSpace(action))
	switch action {
	case "keep", "reassign", "cancel", "dismiss", "mark_for_review":
	default:
	...
	if action == "reassign" {
		...
		// Build snapshot for the new assignment.
		capturedAt := time.Now().UTC()
		capturedAtPg := pgtype.Timestamptz{Time: capturedAt, Valid: true}

		row, err := q.SessionGetByIDForSnapshot(ctx, candidateSessionID)
		if err != nil {
		...
		snapshotJSON, schemaVersion, err := snapshotFunc(
			...
			candidateSessionID, row.SeriesID, row.CourseID, row.RoomID, row.TeacherID,
