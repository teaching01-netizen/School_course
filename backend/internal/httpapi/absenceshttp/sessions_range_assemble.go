...
// passes the bundle resolver. Order contract (Step 18): output follows the
// `order` slice (first-seen course order from SQL ORDER BY sub.code,
// start_at), and sessions within a course follow fact order — never map
// iteration. This matches the legacy courseOrder append-on-first-sight loop.
