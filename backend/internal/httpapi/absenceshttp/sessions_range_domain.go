...
// The request clock arrives as an argument (timing filter `now`);
// instantiating it is the caller job (dispatch captures one now per
// request, Step 18). Complexity notes are inline; nothing here may issue
// a query.
//
// Honest complexity (Step 18): per-function costs below are linear in
// their inputs, but the ENDPOINT is not O(R). Sorting (enrolled-level
// order, priority labels) costs O(R log R); scope metadata, historical
// day counts, and the sit-in bundle are additional inputs beyond the
// display rows R. Do not advertise endpoint O(R) while omitting them.
...
func mergedRangesFromSiblings(facts, extraSiblings []sessionFact, instituteTZ string) map[string][2]string {
	...
	// Map iteration order is nondeterministic, but each key computes its
	// value independently (no accumulation across keys), so the OUTPUT MAP
	// is identical regardless of visit order. Callers needing stable
	// serialization must sort keys; JSON marshaling of maps already sorts
	// string keys. (Step 18 audit: no order hazard here.)
...
// call; callers build a map once for O(1) amortized lookup. Last-write-wins
// on duplicate course coverage matches the legacy per-course overwrite
// order only when scopes arrive in the same order; the batched scope query
// has no ORDER BY (Step 18 audit: overlapping-scope order is loader-defined
// on both paths — the tail-batch fold preserves standalone scan order).
