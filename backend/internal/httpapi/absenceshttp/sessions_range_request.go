...
type sessionsRangeLookup interface {
	...
	// on every other path by construction.
	isLifetime() bool
}
...
// represent the capability, so no execution path can upgrade authority
// via query parameters (same pattern as bypassTiming).
func (l StudentSessionLookup) isLifetime() bool { return false }
...
func finalizeSessionsRangeLookup(
	...
	// Step 17: explicit authorized lifetime staff lookup. lifetime=true
	// (presence with a true value) skips the range cap for ADMIN requests
	// only: it must be combined with date_from+date_to, and it never
	// grants anything to students (the student handler rejects presence
	// before reaching here; non-admin staff lookups ignore it below).
	// Malformed values fail closed with bad_lifetime.
	lifetime := false
	...
	// Student-identity calls (forced wcode from session state, non-admin)
	// must never use the lifetime path even if smuggled past the
	// self-service presence guard (enforced at construction below: the
	// student type cannot represent Lifetime, and non-admin lifetime is
	// rejected outright).
	if lifetime && !pre.rangeProvided {
	...
	// The lifetime capability lands only on admin lookups: a non-admin
	// request with lifetime=true is rejected above (bad_lifetime), and
	// the student type cannot represent it regardless.
	lifetimeOK := lifetime && pre.adminRequest
	if lifetime && !pre.adminRequest {
	...
	if includeAllSubjects {
		...
		return StaffAllSubjectsLookup{WCode: pre.wcode, SubjectIDs: subjectIDFilter, CourseIDs: allowedCourseIDs, BypassTiming: bypassTiming, SatAfterPriority: satVerbalAfterPriority, Lifetime: lifetimeOK}, true
	}
