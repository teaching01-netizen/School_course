...
// now is the request clock (Step 18: captured once at dispatch; cutoffs
// and SAT RequestTime derive from it, never time.Now).
...
func resolveSitInForCourseFromBundle(in bundleSitInInputs, wcode string, studentID pgtype.UUID, courseID, subjectID pgtype.UUID, dateFrom, dateTo time.Time) (*SitInResult, error) {
	...
	cutoff := time.Time{}
	if win > 0 {
		cutoff = in.now.Add(time.Duration(win) * 7 * 24 * time.Hour)
	}
