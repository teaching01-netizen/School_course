...
type Session = { id: string; course_id: string; room_id: string | null; teacher_id: string; start_at: string; end_at: string };
...
export default function Reports() {
  ...
  const roomUtil = useMemo(() => {
    ...
    for (const s of sessions) {
      ...
      const row = byRoom.get(key) ?? { sessions: 0, minutes: 0 };
      row.sessions += 1;
    ...
    return Array.from(byRoom.entries())
      .map(([roomId, v]) => {
        ...
        return { id: roomId, name: r?.name ?? (roomId === 'unassigned' ? 'Unassigned' : roomId), capacity: r?.capacity ?? null, sessions: v.sessions, hours: v.minutes / 60 };
      })
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="px-2 py-1 text-sm border border-[var(--color-wi-line)] rounded-sm" />
        <span className="text-sm text-[var(--color-wi-text-light)]">to</span>
        <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="px-2 py-1 text-sm border border-[var(--color-wi-line)] rounded-sm" />
        <Button variant="primary" size="md" onClick={() => void load()}>Reload</Button>
        {loading ? <LoadingSkeleton type="table" lines={3} /> : null}
      </div>

      <div className="flex items-center gap-1 mb-4 border-b border-b-[var(--color-wi-line)]">
        {[
      ...
      {activeReport === 'daily' && (
        <div>
          <div className="grid grid-cols-3 gap-4 mb-4">
            <div className="bg-[var(--color-wi-row-alt)] border border-[var(--color-wi-line)] p-3 rounded-sm">
          ...
          <div className="overflow-x-auto"><table className="w-full text-[13px]">
            ...
            <tbody>
              {roomUtil.map((r) => (
                <tr key={r.name} className="border-b border-b-[var(--color-wi-line)] hover:bg-[var(--color-wi-row-alt)]">
                  <td className="py-2 px-2">{r.name}</td>
                  <td className="py-2 px-2">{r.sessions}</td>
                  <td className="py-2 px-2">{r.hours.toFixed(1)}h</td>
                  <td className="py-2 px-2">
                    <div className="flex items-center gap-2">
                      <div className="w-24 h-2 bg-[var(--color-wi-row-alt)] rounded-sm overflow-hidden">
                        <div className="h-full bg-[var(--color-wi-primary)]" style={{ width: `${Math.min(100, (r.hours / 40) * 100)}%` }} />
                      </div>
                      <span className="text-xs text-[var(--color-wi-text-light)]">{Math.round((r.hours / 40) * 100)}%</span>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {activeReport === 'classrooms' && (
        <div className="overflow-x-auto"><table className="w-full text-[13px]">
          ...
          <tbody>
            {roomUtil.map((r) => (
              <tr key={r.id} className="border-b border-b-[var(--color-wi-line)] hover:bg-[var(--color-wi-row-alt)]">
                <td className="py-2 px-2">{r.name}</td>
      ...
      {activeReport === 'courses' && (
        <div className="overflow-x-auto"><table className="w-full text-[13px]">
          ...
          <tbody>
            {courseSummary.map((c) => (
              <tr key={c.id} className="border-b border-b-[var(--color-wi-line)] hover:bg-[var(--color-wi-row-alt)]">
      ...
      {activeReport === 'absences' && (
        <div>
          <div className="bg-[var(--color-wi-row-alt)] border border-[var(--color-wi-line)] p-4 rounded-sm">
