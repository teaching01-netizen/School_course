...
import { parseConflictOverviewResponse, scheduleConflictsURL } from "@/features/scheduling/api/conflictOverviewApi";
...
export default function ScheduleConflicts() {
  ...
  return <div>
    ...
    {parsed ? <ConflictSummary summary={parsed.summary} /> : <LoadingSkeleton type="card" lines={4} />}
    <ConflictFilters
      query={searchInput}
    ...
    {query.loading && parsed === null ? <LoadingSkeleton type="table" lines={6} /> : query.error && parsed === null ? (
      ...
      <div className="mt-3 flex items-center justify-between gap-3 text-sm text-[var(--color-wi-text-light)]"><span>{parsed.total_count} {parsed.total_count === 1 ? "conflict" : "conflicts"}</span><div className="flex gap-2"><Button variant="secondary" size="sm" disabled={offset === 0} onClick={() => updateFilter("offset", String(Math.max(0, offset - parsed.limit)))}>Previous</Button><Button variant="secondary" size="sm" disabled={offset + parsed.items.length >= parsed.total_count} onClick={() => updateFilter("offset", String(offset + parsed.limit))}>Next</Button></div></div>
    </> : null}
  </div>;
}
