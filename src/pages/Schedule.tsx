        ...
        <div className="border border-[var(--color-wi-line)] rounded-sm overflow-hidden">
          <div className="grid" style={{ gridTemplateColumns: `repeat(${Math.max(daysInRange.length, 1)}, minmax(0, 1fr))` }}>
            {daysInRange.map((d) => {
              ...
              return (
                <div key={key} className="border-r border-r-[var(--color-wi-line)] last:border-r-0">
                  <div className="bg-[var(--color-wi-row-alt)] border-b border-b-[var(--color-wi-line)] px-3 py-2">
                  ...
                  <div className="p-2 space-y-2">
                    {items.length === 0 ? (
                      ...
                      items.map((s) => {
                        ...
                        return (
                          <div key={s.id} className="w-full text-left border border-[var(--color-wi-line)] rounded-sm px-2 py-2 hover:bg-[var(--color-wi-row-alt)]">
        ...
        <div className="overflow-x-auto"><table className="w-full text-[13px]">
          ...
          <tbody>
            {sessions.map((s) => {
              ...
              return (
                <Fragment key={s.id}>
                  <tr key={s.id} className="border-b border-b-[var(--color-wi-line)] hover:bg-[var(--color-wi-row-alt)]">
                  ...
                  {isInlineEditing && (
                    <tr key={`${s.id}-inline`} className="border-b border-b-[var(--color-wi-line)] bg-[var(--color-wi-row-alt)]">
        ...
        >
          {editSeriesLoading || !editSeriesForm ? (
            <div className="py-6 text-center text-sm text-[var(--color-wi-text-light)]">
              <span className="inline-block w-4 h-4 border-2 border-[var(--color-wi-line)] border-t-transparent rounded-full animate-spin mr-2" aria-hidden="true" />
              Loading…
            </div>
          ) : (
            <div className="space-y-6">
              <div className="rounded-sm border border-[var(--color-wi-line)] bg-[var(--color-wi-row-alt)] px-3 py-2 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <FormField name="es-course" label="Course (fixed)">
                  <div className="px-2 py-1.5 text-sm border border-[var(--color-wi-line)] rounded-sm bg-[var(--color-wi-row-alt)]">{courseById.get(editSeriesForm.course_id)?.code ?? editSeriesForm.course_id}</div>
                </FormField>
                <FormField name="es-room" label="Room (fixed)">
                  <div className="px-2 py-1.5 text-sm border border-[var(--color-wi-line)] rounded-sm bg-[var(--color-wi-row-alt)]">{editSeriesForm.room_id ? (roomById.get(editSeriesForm.room_id)?.name ?? editSeriesForm.room_id) : "[NOT SET]"}</div>
                </FormField>
                <FormField name="es-teacher" label="Teacher (fixed)">
                  <div className="px-2 py-1.5 text-sm border border-[var(--color-wi-line)] rounded-sm bg-[var(--color-wi-row-alt)]">{teacherById.get(editSeriesForm.teacher_id)?.full_name || teacherById.get(editSeriesForm.teacher_id)?.username || editSeriesForm.teacher_id}</div>
                </FormField>
              </div>
              <SeriesFormFields
                weekdays={editSeriesForm.weekdays}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-2 py-1.5 text-sm border border-[var(--color-wi-line)] rounded-sm disabled:bg-[var(--color-wi-row-alt)]"
                  />
                </div>
              </div>
