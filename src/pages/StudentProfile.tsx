  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Profile Card */}
        <div className="border border-[var(--color-wi-line)] rounded-sm bg-white p-4">
        ...
        {/* Enrolled Courses */}
        <div className="border border-[var(--color-wi-line)] rounded-sm bg-white p-4 lg:col-span-2">
      ...
      {/* Weekly Schedule */}
      <div className="border border-[var(--color-wi-line)] rounded-sm bg-white p-4 mt-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              ...
              className="px-2 py-1 text-xs border border-[var(--color-wi-line)] rounded-sm hover:bg-[var(--color-wi-row-alt)] font-medium text-[var(--color-wi-primary)]"
            >
              Today
            </button>
            <button
        ...
        {enrolledCourses.length === 0 ? (
          ...
          <div>
            <div className="overflow-x-auto"><table className="w-full text-[12px] border border-[var(--color-wi-line)]">
              <caption className="sr-only">Weekly schedule</caption>
              <thead>
                <tr className="bg-[var(--color-wi-row-alt)]">
                  <th scope="col" className="text-left py-1 px-1 font-semibold border-r border-r-[var(--color-wi-line)] w-12">Time</th>
                {days.map((d) => (
                  <th scope="col" key={d} className="text-center py-1 px-1 font-semibold border-r border-r-[var(--color-wi-line)] min-w-[100px]">{d}</th>
                ))}
                </tr>
              </thead>
              <tbody>
                {timeSlots.map((slot) => (
                  <tr key={slot} className="border-b border-b-[var(--color-wi-line)]">
                    <td className="py-1 px-1 text-xs text-[var(--color-wi-text-light)] font-medium border-r border-r-[var(--color-wi-line)]">{slot}</td>
                    {[1, 2, 3, 4, 5].map((day) => {
                      const sessList = sessionsByWeekdayAndHour.get(`${day}-${slot}`) ?? [];
                      return (
                        <td key={day} className="px-1 py-1 border-r border-r-[var(--color-wi-line)] align-top">
                          {sessList.length > 0 ? (
