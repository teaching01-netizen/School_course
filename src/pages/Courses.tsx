...
export default function Courses() {
  ...
  const url = `/api/v1/courses${includeArchived ? "?include_archived=1" : ""}`;
  const { data: courses, loading, error } = useApiQuery<CourseRow[]>(url);
          ...
          <tbody>
            {filtered.map((course) => (
              <tr key={course.id} className="border-b border-gray-200 hover:bg-gray-50">
                {(() => {
                  const status = course.deleted_at ? "Archived" : "Active";
                  return (
                    <>
