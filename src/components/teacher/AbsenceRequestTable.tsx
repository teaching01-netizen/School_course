...
function useAbsenceRows(sessions: TeacherDashboardSession[], zone: string): AbsenceRow[] {
  return useMemo(() => {
    ...
    for (const s of sessions) {
      for (const a of s.absent_students ?? []) {
        ...
        rows.push({
          ...
          submittedLabel: formatUTCToZone(createdAt, zone, 'd MMM, HH:mm') ?? createdAt,
          submittedAt: createdAt,
          absenceId: a.absence_id,
          missedSubject: s.subject_name ?? s.course_name,
          missedTimeLabel: `${formatUTCToZone(s.start_at, zone, 'HH:mm') ?? '--:--'}–${formatUTCToZone(s.end_at, zone, 'HH:mm') ?? '--:--'}`,
          sitInSubject: sitIn ? (sitIn.visitedSession.subject_name ?? sitIn.visitedSession.course_name) : null,
          sitInTimeLabel: sitIn ? `${formatUTCToZone(sitIn.visitor.session_start_at, zone, 'HH:mm') ?? '--:--'}–${formatUTCToZone(sitIn.visitor.session_end_at, zone, 'HH:mm') ?? '--:--'}` : null,
          missedDateLabel: formatUTCToZone(s.start_at, zone, 'd MMM') ?? s.start_at.slice(0, 10),
        });
      }
    }
...
export default function AbsenceRequestTable({ sessions, zone }: AbsenceRequestTableProps) {
  const rows = useAbsenceRows(sessions, zone);
