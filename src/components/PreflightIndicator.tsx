...
export function PreflightBadge({ status, details, loading }: PreflightBadgeProps) {
  if (loading) return (
    <span className="flex items-center gap-1 text-xs text-[var(--color-wi-text-light)]">
      <span data-testid="preflight-spinner-badge" className="inline-block w-2.5 h-2.5 border-2 border-[var(--color-wi-line)] border-t-transparent rounded-full animate-spin" aria-hidden="true" />
      Checking…
    </span>
  );
        ...
        <div aria-live="polite" aria-atomic="true">
          {preflight.loading ? (
            <div className="flex items-center gap-1.5 text-[var(--color-wi-text-light)]">
              <span data-testid="preflight-spinner" className="inline-block w-3 h-3 border-2 border-[var(--color-wi-line)] border-t-transparent rounded-full animate-spin" aria-hidden="true" />
