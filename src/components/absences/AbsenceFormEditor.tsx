...
export function AbsenceFormEditor({
  ...
  const inputClass = "mt-1 block w-full rounded-sm border border-wi-line p-2 text-sm";
  ...
  return (
    <div className="space-y-4">
      {showFormBehavior && (
        <section className="rounded-sm border border-wi-line bg-white p-5">
                ...
                {settings.form.reason_categories.map((category, index) => (
                  <div className="flex gap-2" key={`${category.value}-${index}`}>
                    <input
                      ...
                      className="w-1/3 rounded-sm border border-wi-line p-2 text-sm"
                      placeholder="value"
                    ...
                    <input
                      ...
                      className="flex-1 rounded-sm border border-wi-line p-2 text-sm"
          ...
          {showTextEditors && (
            <>
              <label className="mt-5 block text-sm">
                ...
                <textarea className="mt-1 block w-full rounded-sm border border-wi-line p-2 text-sm" maxLength={500} rows={3} value={settings.form.intro_text} onChange={(e) => onChange({ ...settings, form: { ...settings.form, intro_text: e.target.value } })} />
              </label>
              <label className="mt-4 block text-sm">
                Confirmation text shown after submission
                <textarea className="mt-1 block w-full rounded-sm border border-wi-line p-2 text-sm" maxLength={500} rows={2} value={settings.form.confirmation_text} onChange={(e) => onChange({ ...settings, form: { ...settings.form, confirmation_text: e.target.value } })} />
              </label>
            </>
          )}
        </section>
      ...
      {showSitInSection && (
        <section className="rounded-sm border border-wi-line bg-white p-5">
          ...
          <label className="mt-4 block text-sm">
            ...
            <input className="mt-1 block w-32 rounded-sm border border-wi-line p-2 text-sm" min={1} max={100} type="number" value={settings.sit_in.max_sessions_per_absence} onChange={(e) => onChange({ ...settings, sit_in: { ...settings.sit_in, max_sessions_per_absence: Number(e.target.value) } })} />
          </label>
        </section>
      )}
        ...
        <section className="rounded-sm border border-wi-line bg-white p-5">
          ...
          <div className="space-y-4">
            ...
            <label className="block text-sm">
              ...
              <textarea className="mt-1 block w-full rounded-sm border border-wi-line p-2 text-sm" maxLength={500} rows={3} value={settings.notifications?.sms_parent_template ?? ""}               onChange={(e) => onChange({ ...settings, notifications: notificationsWith({ sms_parent_template: e.target.value }) })} />
        ...
        <section className="rounded-sm border border-wi-line bg-white p-5">
          ...
          <div className="space-y-4">
            ...
            <label className="block text-sm">
              Email body (HTML)
              <textarea
                className="mt-1 block w-full rounded-sm border border-wi-line p-2 text-sm font-mono"
                maxLength={15000}
                rows={10}
