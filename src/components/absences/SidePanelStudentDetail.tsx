...
export default function SidePanelStudentDetail({ absence, absences, dayLabel, onBack }: SidePanelStudentDetailProps) {
  ...
  return (
    <div className="space-y-4">
      <button type="button" className="text-xs font-medium text-[var(--color-wi-text-light)] hover:text-[var(--color-wi-text)]" onClick={onBack}>
        {dayLabel} &gt; {absence.student_name?.trim() || absence.wcode}
      </button>

      <section className="flex items-center gap-3 rounded-sm border border-wi-line-soft bg-[var(--color-wi-row-alt)] p-3">
        ...
        <div className="min-w-0">
          <h3 className="truncate text-sm font-semibold text-[var(--color-wi-text)]">{getAbsenceStudentLabel(absence)}</h3>
          <p className="text-xs text-[var(--color-wi-text-light)]">Focused absence and sit-in history for this calendar range.</p>
        </div>
      </section>

      <section>
        <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--color-wi-text-light)]">Absence History ({studentAbsences.length})</h4>
        {studentAbsences.length === 0 ? (
          <EmptyState message="No absences recorded for this student in this range." />
        ) : (
          <div className="space-y-2">
            {studentAbsences.map((item) => (
              <article key={item.id} className="rounded-sm border border-wi-line-soft bg-white p-3 text-sm shadow-sm">
      ...
      <section>
        <h4 className="mb-2 text-xs font-semibold uppercase tracking-wide text-[var(--color-wi-text-light)]">Upcoming Sit-ins ({sitInSessions.length})</h4>
        {sitInSessions.length === 0 ? (
          ...
          <div className="space-y-2">
            {sitInSessions.map((session) => (
              <article key={`${session.id}-${session.start_at}`} className="rounded-sm border border-wi-line-soft bg-white p-3 text-sm shadow-sm">
