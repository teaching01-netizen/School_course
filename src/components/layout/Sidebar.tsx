...
export default function Sidebar({ collapsed, mobileOpen, onCloseMobile }: SidebarProps) {
  ...
  const content = (
    <div className="relative flex h-full min-h-0 flex-col">
      ...
      <div className="shrink-0 border-t border-t-[var(--color-wi-line)] px-2 py-1.5">
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside
        className={`relative z-30 hidden shrink-0 self-start border-r border-r-[var(--color-wi-line)] bg-wi-bg transition-[width] duration-150 motion-reduce:transition-none md:block ${
      ...
      >
        ...
        {!collapsed && (
          <div
            ...
            onKeyDown={(e) => {
              if (e.key === "ArrowLeft") { e.preventDefault(); setWidth((v) => Math.max(MIN_WIDTH, v - 10)); }
              else if (e.key === "ArrowRight") { e.preventDefault(); setWidth((v) => Math.min(MAX_WIDTH, v + 10)); }
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-40 md:hidden">
          <div
            ...
            aria-hidden="true"
          />
          <aside className="absolute inset-y-0 left-0 z-10 w-[280px] max-w-[85vw] border-r border-r-[var(--color-wi-line)] bg-wi-bg shadow-[10px_0_32px_rgba(0,0,0,0.08)]">
